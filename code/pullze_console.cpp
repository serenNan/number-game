#include "pullze.h"
#include <cstdio>  // 添加sprintf函数的头文件
#include <cstring> // 添加字符串处理函数的头文件
...
void display_matrix_graphic()
{
    ...
    // 设置单元格大小
    int cell_width = g_has_separators ? 5 : 2; // 有分隔线时每个单元格占5列，否则占2列

    // 计算矩阵大小
    int matrix_width = g_cols * cell_width + 1;
    int matrix_height =
        g_has_separators ? g_rows * 3 + 1 : g_rows + 2; // 有分隔线时每行占3行，否则占1行
...
void convert_mouse_to_cell(int mx, int my, int &row, int &col, bool &is_valid)
{
    // 设置单元格大小
    int cell_width = g_has_separators ? 5 : 2;  // 有分隔线时每个单元格占5列，否则占2列
    int cell_height = g_has_separators ? 3 : 1; // 有分隔线时每个单元格占3行，否则占1行
